  ...
  fetch('./data/glamis_tileset.geojson')
    ...
    .then(data => {
      ...
      Promise.all(imagePromises).then(() => {
        ...
        // Auto-show China Wall popup when map loads
        showChinaWallPopup();
      });
    })
    .catch(error => {
...
function setupMapInteractions() {
  ...
  map.on('click', 'glamis-points-layer', function(e) {
    ...
    // Only allow popup for China Wall
    if (props.name !== 'China Wall') {
      return;
    }
    ...
    // Calculate more generous offset to ensure popup is fully visible
    const offsetY = mapHeight * 0.25; // Increase offset to 25% of viewport height
    const offsetX = 0; // Center horizontally
    
    map.easeTo({
      center: coords,
      offset: [offsetX, offsetY], // Shift center down significantly to prevent popup cutoff
      duration: 800,
      essential: true
    });
    ...
    // Additional check to ensure popup is fully visible after rendering
    setTimeout(() => {
      ...
      if (popupElement) {
        ...
        if (isOutsideTop || isOutsideBottom || isOutsideLeft || isOutsideRight) {
          ...
          if (isOutsideTop) {
            additionalOffsetY = (mapRect.top + 20 - popupRect.top) / 2;
          }
          if (isOutsideBottom) {
            additionalOffsetY = (mapRect.bottom - 20 - popupRect.bottom) / 2;
          }
          if (isOutsideLeft) {
            additionalOffsetX = (mapRect.left + 20 - popupRect.left) / 2;
          }
          if (isOutsideRight) {
            additionalOffsetX = (mapRect.right - 20 - popupRect.right) / 2;
          }
          
          // Fine-tune position
          map.easeTo({
            center: coords,
            offset: [offsetX + additionalOffsetX, offsetY + additionalOffsetY],
            duration: 300,
            essential: true
          });
        }
      }
    }, 200); // Check after popup is rendered
  });
...
function addPointsLayers() {
  ...
  // Labels removed - no location names will be displayed

  // Move selected points layer to top
  map.moveLayer('glamis-points-selected');
  
  setupMapInteractions();
}

// Function to show China Wall popup when map loads
function showChinaWallPopup() {
  // Wait a moment for layers to be fully rendered
  setTimeout(() => {
    // Fetch China Wall data from GeoJSON directly
    fetch('./data/glamis_tileset.geojson')
      .then(response => response.json())
      .then(data => {
        const chinaWallFeature = data.features.find(feature => 
          feature.properties.name === 'China Wall'
        );
        
        if (chinaWallFeature) {
          const coords = chinaWallFeature.geometry.coordinates.slice();
          const props = chinaWallFeature.properties;
          
          // Update pin selection
          updatePinSelection(props.name);
          
          // Pan map to center the location with offset for popup
          const mapContainer = map.getContainer();
          const mapHeight = mapContainer.offsetHeight;
          const mapWidth = mapContainer.offsetWidth;
      ...
      .catch(error => {
        console.error('Error loading China Wall data:', error);
      });
  }, 500); // Wait 500ms for layers to be fully rendered
}
